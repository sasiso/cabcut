...
def createSpheresOnPerimeterLine(polyline, spacing_pixels=10, radius=2.0):
    ...
    for i in range(num_points_total - 1):
        ...
        p1 = points.GetPoint((i + 1) % num_points_total)  # Wrap around to connect last point to first
...
def createPolyLineWithPlanesFromSpheresToCircle(sphere_polydata, circle_polydata):
    ...
    # Create points for polyline and planes
    points = vtk.vtkPoints()
    ...
    for i in range(num_spheres):
        ...
        # Calculate next sphere index (wrap around to the first sphere for last sphere)
        next_sphere_index = (i + 1) % num_spheres
        next_sphere_point = sphere_points.GetPoint(next_sphere_index)
        
        # Calculate plane points
        plane_points = vtk.vtkPoints()
        plane_points.InsertNextPoint(sphere_point)
        plane_points.InsertNextPoint(circle_center)
        plane_points.InsertNextPoint(next_sphere_point)
        plane_points.InsertNextPoint(circle_center)
        ...
        for j in range(4):
            polygon.GetPointIds().SetId(j, points.InsertNextPoint(plane_points.GetPoint(j)))
        polygons.InsertNextCell(polygon)
        
        # Create line segment from current sphere to circle center
        line = vtk.vtkLine()
        line.GetPointIds().SetId(0, points.InsertNextPoint(sphere_point))
        line.GetPointIds().SetId(1, points.InsertNextPoint(circle_center))
        lines.InsertNextCell(line)
    
    # Create polydata for polyline with planes
    polydata = vtk.vtkPolyData()
    polydata.SetPoints(points)
...
def main():
    ...
    polyline_polydata = createPolyLineWithPlanesFromSpheresToCircle(sphere_polydata, circle_polydata)
